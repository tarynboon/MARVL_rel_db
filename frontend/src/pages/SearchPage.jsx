  ...
  return (
    <div className="space-y-4">
      ...
      {total > filters.limit && (() => {
        ...
        const goToPage = (p) => setFilters(f => ({ ...f, offset: (p - 1) * f.limit }));
        const pageBtn = 'px-3 py-2 text-sm rounded-lg border disabled:opacity-40';
          ...
          <div className="space-y-2">
            <div className="flex justify-center flex-wrap gap-1.5">
              <button
                ...
                onClick={() => goToPage(1)}
                className={`${pageBtn} border-slate-200 hover:bg-slate-100`}
              >
                Start
              </button>
              {pageList(currentPage, totalPages).map((p, i) =>
                p === '…' ? (
                  <span key={`gap-${i}`} className="px-2 py-2 text-sm text-slate-400">…</span>
                ) : (
                  <button
                    key={p}
                    onClick={() => goToPage(p)}
